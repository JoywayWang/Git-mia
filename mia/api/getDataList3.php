...
{
  # 获取所有的商品信息
  $sql = "SELECT * FROM `good3` order by `gid` limit $page , 20";
}else if($typeOrder == 1)
{
  $sql = "SELECT * FROM `good3` ORDER BY `good3`.`sale_price` DESC limit $page , 20";
}else if($typeOrder == 2)
{
  $sql = "SELECT * FROM `good3` ORDER BY `good3`.`sale_price` ASC limit $page , 20";
}
